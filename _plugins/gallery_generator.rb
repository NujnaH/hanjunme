        ...
        def initialize(site, base, dir, gallery_name)
            ...
            Dir.foreach(dir) do |image|
                if image.chars.first != "." and image.downcase().end_with?(*$image_extensions)
                    @images.push(image)
                    imageTitle = image
                    # the each do method spits the item in the array into the variable defined in the | x | block. 
                    $image_extensions.each do |extension|
                        imageTitle = imageTitle.sub(extension, "")
                    end
                    imageTitle = imageTitle.gsub("_", " ").gsub(/\w+/) {|word| word.capitalize}
            ...
            self.data["imageTitles"] = @imageTitles
            begin
